...
задел на следующую волну. **Аналитика**: PostHog (`js/analytics.js`), самоскрывающийся как
лидерборд — пустой `POSTHOG_KEY` в `config.js` → модуль не грузит скрипт вообще; трекает
старт/конец забега (причина смерти, биом, счёт), тюториал, пермишен на датчики, открытие
фич, JS-ошибки (клики — автозахватом SDK); включение и рецепт retention-по-квартилям —
`docs/ANALYTICS_SETUP.md`. Экономика: монеты на бревне → магазин 🛒 (аватар-бревно,
...
- Статический DOM/CSS (не сам игровой цикл) можно всё же проверить через `mcp__Claude_Preview`:
  превью-процесс не видит `/Users/...` — правь `.claude/launch.json` на `python3 <scratchpad>/serve.py`,
  который отдаёт rsync-копию сайта из скретчпада (пересобирать под ID новой сессии — старый путь
  мёртв). Вкладка превью «hidden» → `requestAnimationFrame` не тикает, реальный игровой цикл
  (спавны, таймеры, паденение по elapsed) там не идёт; но DOM-классы/стили можно дёргать напрямую
  через `dynamic import()` нужного модуля + `preview_eval` и смотреть скриншотом/`preview_inspect` —
  этого достаточно для CSS-層/UI-багов вроде «плашка не прячется». Правка `launch.json` может
  потребовать спросить пользователя перед `preview_stop` (харнесс иногда блокирует эту пару как
  self-modification tooling).
- **CSS driven от `.on`, но НЕ от `.hidden`, — грабля**: элементы, чья видимость завязана на
  css-класс типа `#layer.on ~ #hint` (см. `#boss-hint`), не спрячутся, если код для «пауз с
  сохранением стейта» (revive) добавляет отдельный `.hidden` класс, не трогая `.on`. Проверяй
  оба класса в селекторе (`.on:not(.hidden)`), если элемент должен молчать между `gameOver` и
  `resumeRun`/новым забегом.
...
| `shop.js` | Магазин 2.0 🛒: **двухуровневые табы оверлея** — верхний ряд «Скины»/«Сундуки»/«Прочее» (`activeGroup`, `#shop-tabs`), внутри «Скины» — под-табы по слотам «Голова»/«Тело»/«Ноги»/«Костюмы» (`activeSlot`, `#shop-subtabs`, видны только когда `activeGroup==='skins'`); «Сундуки» — пока заглушка «скоро» (гача не реализована); «Прочее» — аватар-бревно (`photo_url` клипом в `#log-svg`) + запасное сердце. Реестр `SKINS` в config.js с редкостями (костюм `suit_*` перекрывает индивидуальные слоты). Манекен = замороженный клон `#player`, сетка карточек с рамкой редкости `--rar-*`, клики делегированием (два обработчика — на `#shop-tabs` и на `#shop-subtabs`). Владение — `ownedSkins` CSV (union-merge); **экипировка per-slot** (`stayOnLog_equip{Log,Head,Body,Legs,Suit}`, миграция старого `equippedSkin`; cloud заполняет только пустоту). `applyCharSkin` вешает классы `char-<slot>-<id>`/`char-costume-<id>` на `#player`/`#falling-player`/манекен через **classList** (у SVG `className` — SVGAnimatedString!). `spendCoins` (общий кошелёк-райтер, его же юзает revive; кошелёк только localStorage). |
...
| `analytics.js` | Клиент PostHog: `initAnalytics()`/`track()`, самоскрывающийся как лидерборд
(пустой `POSTHOG_KEY` → модуль не грузит скрипт, `track()` no-op — никогда не может сломать
геймплей). `identify()` по TG uid, когда доступен (вне TG — анонимный device-id от SDK).
Автозахват кликов — из коробки SDK; свои события — `run_start/run_end` (причина смерти,
биом, счёт, пермишены/тюториал), `feature_opened`, `js_error` (`window.onerror`/
`unhandledrejection`, не полагается на автоинструментацию SDK). Настройка — `docs/ANALYTICS_SETUP.md`. |
| `render.js` | Анимация падения в воду + брызги + экран результата (`УПАЛ! Очки` + квип + «+N 🪙» + кнопки, включая revive). |
| `audio.js` | Синтез-SFX (Web Audio): `jump/hit/splash/point/combo/whoosh` + **процедурная музыка** (бас/арпеджио/пад, lookahead-планировщик; `music.setMood(biome)` меняет BPM/лад/плотность по биому, ассетов нет). Mute в localStorage, unmute возвращает музыку сразу. `AudioContext` ленивый; `resumeIfSuspended()` резюмит его не только на жесте (`initAudio()`), но и перед каждым `tone()/noise()` + на `visibilitychange` — иначе подвисший после сворачивания контекст молчал до перезапуска игры. |
...
Персонаж-стикман (`#player`, viewBox 0 0 44 55, рендер ~1.36× = 60×75; клон падения
`#falling-player`; общий хук-класс `.char-fig`) красится переменными `--char-skin/--char-line`
и носит скины: инлайн-`<g class="skin-*">` внутри `.head`/торса/`.leg-*` (едут с анимацией),
тоглятся классами `char-*` от `shop.js`; костюм перекрывает индивидуальные слоты + перекрас.
