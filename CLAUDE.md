...
| `shop.js` | Магазин 2.0 🛒: **скины персонажа** по 4 слотам (голова/тело/ноги — реестр `SKINS` в config.js с редкостями; костюм `suit_*` перекрывает индивидуальные) + аватар-бревно (`photo_url` клипом в `#log-svg`) + запасное сердце — в табах оверлея (манекен = замороженный клон `#player`, сетка карточек с рамкой редкости `--rar-*`, клики делегированием). Владение — `ownedSkins` CSV (union-merge); **экипировка per-slot** (`stayOnLog_equip{Log,Head,Body,Legs,Suit}`, миграция старого `equippedSkin`; cloud заполняет только пустоту). `applyCharSkin` вешает классы `char-<slot>-<id>`/`char-costume-<id>` на `#player`/`#falling-player`/манекен через **classList** (у SVG `className` — SVGAnimatedString!). `spendCoins` (общий кошелёк-райтер, его же юзает revive; кошелёк только localStorage). |
