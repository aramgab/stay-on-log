...
| `boss.js` | Босс-движок: `idle→intro→fight`, два типа боя — **паттерн** (акула: lunge на сторону бревна / bite-прыжок, 7 телеграфированных атак) и **погоня** (косатка: `bossSpeedFactor()` 1→1.3 + снапы-прыжки по таймеру). Триггер — шторм-фаза цикла, раз за цикл. Хиты → общий `registerHit(pos,'boss')`; пока активен — препятствия/монеты не выныривают, смены откладываются; стейт переживает `gameOver` ради revive, полный сброс только в `showCountdown`. Арт — рисованные inline-SVG `#shark-svg`/`#orca-svg` в `#boss-shark` (вертикальный прыжок, пасть к бревну), вариант переключает класс `.orca`, состояния — классы на `#boss-layer`. |
| `map.js` | Карта кампании 🗺 (оверлей в стиле магазина): главы×4 карточки — играбельная (выбор тапом, «✓ пройден ×N»), замок с чеклистом заданий, «скоро», тизер с именем, «?»; секция Событий-заглушка. Кнопка `#map-btn` показывает выбранный мир. |
...
- **Кампания**: биом на весь забег (выбор на карте 🗺; туториал всегда Земля; revive сохраняет
  ...
  max). **Босс** в шторм-фазе каждого цикла: форшадоуинг-плавники с первой секунды (`fins-on`),
