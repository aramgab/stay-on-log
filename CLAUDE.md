...
| `boss.js` | Босс-движок: `idle→intro→fight`, два типа боя — **паттерн** (акула: lunge на сторону бревна / bite-прыжок, 7 телеграфированных атак) и **погоня** (косатка: `bossSpeedFactor()` 1→1.3 + снапы-прыжки по таймеру — уворот-хореография в три такта: в погоне идёт низко за бревном, на телеграфе ПОГРУЖАЕТСЯ и дрожит-заряжается, на страйке свеча-прыжок до кромки бревна с пенным всплеском `#boss-splash`, затем `snap-recover` — нырок обратно и возврат к погоне; прыжок игрока = уворот). Триггер — шторм-фаза цикла, раз за цикл. Хиты → общий `registerHit(pos,'boss')`; пока активен — препятствия/монеты не выныривают, смены откладываются; стейт переживает `gameOver` ради revive, полный сброс только в `showCountdown`. Арт — рисованные inline-SVG `#shark-svg`/`#orca-svg` в `#boss-shark` (вертикальный прыжок, пасть к бревну), вариант переключает класс `.orca`, состояния — классы на `#boss-layer`. |
